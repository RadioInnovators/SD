...
module LFSR_Plus #(parameter W = 12, V = 18, g_type = 0, u_type = 1, SCALOR = 1)      
	(
		output reg signed [W-1 : 0]		g_noise_out,
		output reg signed [W-1 : 0]		g_noise_out_scaled_mean = 0,
		output reg signed [W-1 : 0]		g_noise_out_scaled_mean_var = 0,
...
////---------------- internal variables ----------------------------------
	reg	 signed	[W-1 : 0] 	rand_out;
	//// flip flops for shift registers
	reg	signed	[W-1 : 0]	rand_ff;
	reg	signed	[V-1 : 0]	rand_en_ff;
	//// registers for gaussian distribution, these form bus wide shift registers
	reg  signed	[W-1 : 0]	temp_u_noise3;
	reg  signed	[W-1 : 0]	temp_u_noise2;
	reg  signed	[W-1 : 0]	temp_u_noise1;
	reg  signed	[W-1 : 0]	temp_u_noise0;
	reg  signed	[W-1 : 0]	temp_g_noise_nxt;
	...
	always @(*)
		...
		if(enable == 1)begin
			I = I + 1;
			g_noise_out_scaled_mean_var = g_noise_out / SCALOR;
			 // noise power = var / R
			 // SNR A^2/var
			 // A = 20000
